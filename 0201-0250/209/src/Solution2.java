...
public class Solution2 {
    ...
    public int minSubArrayLen(int s, int[] nums) {
        ...
        for (int i = 0; i < len - 1; i++) {
            ...
            // 设置成一个比较大的数，但是这个数有下界
            // i 的最大值是 len-2,
            // curr_target - i + 1 >= len + 1
            // curr_target >= i + len = 2 * len -2
            int curr_target = 2 * len - 2;
            // int curr_target = 2 * len - 1; 能通过
            // int curr_target = 2 * len - 3; 不能通过
            // 退出循环的条件是 l > r
            while (l <= r) {
                int mid = l + (r - l) / 2;
            ...
            min_len = Math.min(min_len,curr_target - i + 1);
        }
